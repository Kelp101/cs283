...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    if(read(fd, &another_student, STUDENT_RECORD_SIZE) == -1){
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    student_t new_student = EMPTY_STUDENT_RECORD;
    new_student.id = id;
    new_student.gpa = gpa;
    ...
    if(another_student.id != DELETED_STUDENT_ID && another_student.id == id){
...
int del_student(int fd, int id)
{
    ...
    student_t another_student;
    int delete_student = get_student(fd, id, &another_student);

    if(delete_student == SRCH_NOT_FOUND){
        printf(M_STD_NOT_FND_MSG, id);
        return ERR_DB_OP;
    } else if (delete_student == ERR_DB_FILE){
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
